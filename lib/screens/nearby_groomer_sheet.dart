...
// 동물병원 찾기(nearby_vet_sheet.dart)와 동일한 딥링크 패턴.
// 위치 권한 안내·Android 권한 요구사항도 동일하게 적용됨.

const String _kSearchQuery = '애견미용';

enum _MapApp { google, kakao, naver }

Future<void> showNearbyGroomerSheet(BuildContext context) {
  return showModalBottomSheet<void>(
    context: context,
    showDragHandle: true,
    builder: (_) => const _NearbyGroomerSheet(),
  );
}

class _NearbyGroomerSheet extends StatelessWidget {
  const _NearbyGroomerSheet();
  ...
  Future<void> _handle(BuildContext context, _MapApp app) async {
    ...
    if (pos == null) {
      messenger.showSnackBar(
        const SnackBar(
          content: Text('위치를 허용하면 더 가까운 미용실을 찾을 수 있어요.'),
        ),
      );
    }
